...
def extract_text(content_dict, text_type="full"):
    ...
    if text_type == "TA" or text_type == "title_abstract":
        ...
        if "title" in content_dict:
            all_text.append(str(content_dict["title"]))
        if "pa01" in content_dict:
            all_text.append(str(content_dict["pa01"]))
            
        # Add claims and description, converting any non-string to string
        for key, value in content_dict.items():
            if key != "title" and key != "pa01":
                if not isinstance(value, str):
                    value = json.dumps(value)  # Converts dicts to a JSON-formatted string
